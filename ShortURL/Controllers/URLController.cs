...
namespace ShortURL.Controllers
{
    ...
    public class UrlsController : ControllerBase
    {
        ...
        public UrlsController(URLDbContext context, IHttpClientFactory httpClientFactory)
        {
            _context = context;
        ...
        public async Task<IActionResult> ShortenUrl([FromBody] ShortenUrlRequest request)
        {
            ...
            // Ensure only HTTP/HTTPS URLs are accepted and that the URL is reachable
            if ((validatedUri.Scheme != Uri.UriSchemeHttp && validatedUri.Scheme != Uri.UriSchemeHttps) ||
                !await UrlAppearsReachableAsync(validatedUri, HttpContext.RequestAborted))
            {
                return BadRequest(new { error = "The provided URL does not appear to be reachable." });
            }
        ...
        private async Task<bool> UrlAppearsReachableAsync(Uri uri, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    var headResponse = await httpClient.SendAsync(headRequest, HttpCompletionOption.ResponseHeadersRead, ct);
                    ...
                    {
                        // If HEAD is supported but not successful, consider it not reachable
                        // Fall through to GET only if HEAD is not supported
                    }
                    else
                    {
